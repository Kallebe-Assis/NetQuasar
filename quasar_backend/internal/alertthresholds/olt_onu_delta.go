...
func EvaluateOltOnuQuantityDeltaAlerts(
	...
	alertstore.CloseExpired(ctx, pool, alertTypeOltOnuRise, onuRiseInfoTTL)
	...
	for _, p := range curPons {
		...
		if !curOK || !prevOK || prev == curOn {
			if okCnt {
				closeOltOnuDeltaAlert(ctx, pool, log, deviceID, alertTypeOltOnuDrop, "onu_drop_count:"+k, 0)
				closeOltOnuDeltaAlert(ctx, pool, log, deviceID, alertTypeOltOnuRise, "onu_rise_count:"+k, onuRiseInfoTTL)
			}
			if okPct {
				closeOltOnuDeltaAlert(ctx, pool, log, deviceID, alertTypeOltOnuDrop, "onu_drop_pct:"+k, 0)
				closeOltOnuDeltaAlert(ctx, pool, log, deviceID, alertTypeOltOnuRise, "onu_rise_pct:"+k, onuRiseInfoTTL)
			}
			continue
		}
		...
		if curOn < prev {
			...
			closeOltOnuDeltaAlert(ctx, pool, log, deviceID, alertTypeOltOnuRise, "onu_rise_count:"+k, 0)
			closeOltOnuDeltaAlert(ctx, pool, log, deviceID, alertTypeOltOnuRise, "onu_rise_pct:"+k, 0)
			continue
		}
...
func evalOltOnuDeltaSide(
	...
	if sev == "ok" {
		closeOltOnuDeltaAlert(ctx, pool, log, deviceID, alertType, key, 0)
		return
	}
	if alertType == alertTypeOltOnuRise {
		sev = "info"
	}
...
func closeOltOnuDeltaAlert(ctx context.Context, pool *pgxpool.Pool, log *zerolog.Logger, deviceID uuid.UUID, alertType, key string, minAge time.Duration) {
