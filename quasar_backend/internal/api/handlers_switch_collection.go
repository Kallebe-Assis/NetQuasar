...
type switchCollectionResponse struct {
...
func (s *Server) getSwitchCollection(w http.ResponseWriter, r *http.Request) {
	...
	metrics := switchcollect.DefaultSwitchMetrics()
	if err == nil {
		if parsed := mikrotikcollect.ParseMetrics(metricsRaw); len(parsed) > 0 {
			metrics = parsed.MergeWithCatalog(switchcollect.SwitchMetricCatalog, switchcollect.DefaultSwitchMetrics())
		}
	}
	writeJSON(w, http.StatusOK, switchCollectionResponse{
		Metrics:         metrics,
		CollectionSteps: mikrotikcollect.ParseSteps(stepsRaw),
		Catalog:         switchcollect.SwitchMetricCatalog,
		Sections:        switchcollect.SwitchSectionLabels,
...
type patchSwitchCollectionBody struct {
	Metrics         mikrotikcollect.MetricsConfig `json:"metrics"`
	CollectionSteps []mikrotikcollect.Step        `json:"collection_steps"`
}

func (s *Server) patchSwitchCollection(w http.ResponseWriter, r *http.Request) {
	var body patchSwitchCollectionBody
	...
	for _, e := range switchcollect.SwitchMetricCatalog {
	...
	_, err := s.DB().Exec(r.Context(), `
		INSERT INTO settings_switch_collection (id, metrics, collection_steps, updated_at)
	...
	s.appendAuditLog(r.Context(), "settings_switch_collection", "1", "patch", s.actorFromRequest(r), nil, map[string]any{
		"has_enabled": mikrotikcollect.HasEnabledMetricsForCatalog(clean, switchcollect.SwitchMetricCatalog),
		"steps":       len(steps),
	})
	writeJSON(w, http.StatusOK, map[string]any{
		...
		"has_enabled":      mikrotikcollect.HasEnabledMetricsForCatalog(clean, switchcollect.SwitchMetricCatalog),
		"message":          patchSwitchSaveMessage(clean),
	})
}

func patchSwitchSaveMessage(c mikrotikcollect.MetricsConfig) string {
	var missing []string
	for _, e := range switchcollect.SwitchMetricCatalog {
		def, ok := c[e.Key]
		if !ok || !def.Enabled {
