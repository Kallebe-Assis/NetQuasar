...
type Props = {
  ...
  /** Recarrega só o total PPPoE online (SNMP live), sem alterar data da última coleta. */
  onRefreshPppoeOnline?: () => void;
  refreshingPppoeOnline?: boolean;
};
  ...
  return (
    <>
      <div className="mk-noc-kpi-row" style={{ gridTemplateColumns: "repeat(6, minmax(0, 1fr))" }}>
        <KpiCard
          icon={Wifi}
          title="Total de PPPoEs Online"
          headAction={
            props.onRefreshPppoeOnline ? (
              <button
                type="button"
              ...
              >
                <RefreshCw size={12} className={props.refreshingPppoeOnline ? "map-refresh-spin" : undefined} />
              </button>
            ) : undefined
          }
        >
