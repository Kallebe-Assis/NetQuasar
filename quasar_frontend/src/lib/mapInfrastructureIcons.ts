...
const INFRA_SVG_INNER: Record<InfraMapKind, string> = {
  // Alfinete: contorno #0D0663, interior branco (sem fundo circular extra).
  cto: `<path d="M12 22s8-5.5 8-12a8 8 0 1 0-16 0c0 6.5 8 12 8 12z" fill="#fff" stroke="COLOR" stroke-width="1.8" stroke-linejoin="round"/><circle cx="12" cy="10" r="2.6" fill="#fff" stroke="COLOR" stroke-width="1.6"/>`,
...
function infraSvgHtml(kind: InfraMapKind, color: string, size = 28): string {
  const inner = INFRA_SVG_INNER[kind].replaceAll("COLOR", color);
  if (kind === "cto") {
    return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" aria-hidden="true">${inner}</svg>`;
  }
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="${color}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="11" fill="#fff" stroke="rgba(0,0,0,0.28)" stroke-width="1"/><g>${inner}</g></svg>`;
}
...
export function infrastructurePinIcon(kind: InfraMapKind, color?: string | null, label?: string | null): L.DivIcon {
  const fill = kind === "cto" ? CTO_MAP_PIN_COLOR : color?.trim() || DEFAULT_INFRA_MAP_COLORS[kind];
  const labelKey = label?.trim() ? label.trim().slice(0, 48) : "";
  const key = `infra:v4:${kind}:${fill}:${labelKey}`;
  const cached = infraIconCache.get(key);
  if (cached) return cached;
  const pin = infraSvgHtml(kind, fill, kind === "cto" ? 22 : 26);
  const labelHtml =
    kind === "cto" && labelKey
  ...
  const iconH = labelHtml ? 38 : kind === "cto" ? 22 : 26;
  const icon = L.divIcon({
    className: "map-infra-pin-wrap",
    html,
    iconSize: [96, iconH],
    iconAnchor: [48, labelHtml ? iconH - 2 : kind === "cto" ? 22 : 13],
    popupAnchor: [0, labelHtml ? -iconH + 4 : kind === "cto" ? -20 : -13],
  });
