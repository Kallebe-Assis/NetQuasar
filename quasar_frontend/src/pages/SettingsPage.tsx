...
function AutomationPanel() {
  ...
  const cfg = useQuery({
    ...
    queryFn: () =>
      ...
      }>("/api/v1/settings/automation/onu-monthly-report"),
  });
  const runs = useQuery({
    queryKey: ["automation-onu-runs"],
    queryFn: () => apiFetch<{ runs: { id: string; started_at: string; status: string | null; summary: unknown }[] }>("/api/v1/settings/automation/onu-monthly-report/runs"),
  });
  ...
  useEffect(() => {
    ...
    setDom(cfg.data.day_of_month != null ? String(cfg.data.day_of_month) : "");
    setDow(cfg.data.day_of_week != null ? String(cfg.data.day_of_week) : "");
    setTh(cfg.data.time_hhmm);
  }, [cfg.data]);
    ...
    mutationFn: () =>
      apiFetch("/api/v1/settings/automation/onu-monthly-report", {
        ...
        json: {
          enabled,
          mode: mode || undefined,
          day_of_month: dom ? Number(dom) : undefined,
          day_of_week: dow ? Number(dow) : undefined,
          time_hhmm: th || undefined,
        },
      }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["automation-onu"] });
      setSaveToast({ ok: true, text: "Guardado com sucesso (agendamento)." });
    },
    onError: (err) => setSaveToast({ ok: false, text: (err as Error).message || "Falha ao guardar (agendamento)." }),
  });

  const run = useMutation({
    mutationFn: () => apiFetch("/api/v1/settings/automation/onu-monthly-report/run", { method: "POST", json: {} }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["automation-onu"] });
      qc.invalidateQueries({ queryKey: ["automation-onu-runs"] });
    },
  });
